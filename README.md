...
## Working with data
1. **Building the warehouse from the raw Excel exports** – Place the source workbooks in `data/` and run `python prepare_warehouse_new.py`. It calls `build_warehouse_new.py` and `build_links.py` as needed and produces the `merged_sales_customers_clean` table with these columns: `meterid`, `customer_no`, `od_date` (DATE), `ocd_paymoney`, `ocd_energy`, `ocd_cash_received`, `utility`, `tariff_type`, plus any customer attribute columns found in the merged table (names containing *name*, *address*, *phone*, *contact*, *town*, *area*, *region*, *location*, *status*, ...). Those attributes appear on the account page; they may be empty for meters that did not match the customer list. To get them onto a hosted instance, export the clean table to CSV and upload it with *Replace the whole dataset*.
2. **Persisted warehouse** – DuckDB lives on disk (`DB_PATH`), so the app starts instantly and nothing is rebuilt at startup.
3. **Uploading CSVs** – Use **Update data** in the navigation (or `/upload`). The CSV must contain the date column; other columns are matched by name (case-insensitive) and cast to the table's types. In *Add new rows* mode, rows that already exist are skipped (only the part of the table overlapping the file's date range is compared, so a monthly update of a multi-year table is quick) and columns the table does not have are ignored. *Replace the whole dataset* builds the new table beside the old one and swaps them when the whole file is in, so the dashboard keeps serving the old data meanwhile and a failed load changes nothing. Files are loaded in slices of about 48 MB, which keeps memory bounded on small hosted instances; `/health` answers immediately while a load runs (`busy: true`).
...
- **Watch list** (private view only) – Every meter scored with the same rules as the account page, rebuilt automatically whenever the data changes: no recent purchases (silent 4 to 12 months, 3 points), consumption drop (last 3 months under half of the same months a year earlier, 2 points), low versus peers (under 30% of the median for the same account type and district, 2 points), inactive over a year (1 point). Sort by score, by kWh below expected (the larger of the fall from the prior 12 months and the gap to the peer median) or by time since the last purchase; filter by district, account type, signal and inspection state; download the ranked list as CSV for inspection teams.
- **Inspection outcomes** – On each account page officers record what a visit found (bypass or tampering, nothing, meter faulty, property vacant, other) with a note and their name. Outcomes are stored in the warehouse beside the data, survive dataset replacements, appear on the watch list, and are the training data for a learned model later.
