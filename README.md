...
3. **Uploading CSVs** – Use **Update data** in the navigation (or `/upload`). The CSV must contain the date column; other columns are matched by name (case-insensitive) and cast to the table's types. In *Add new rows* mode, rows that already exist are skipped and columns the table does not have are ignored. *Replace the whole dataset* drops the table and recreates it from the file. With no dataset present, the upload creates the table.
